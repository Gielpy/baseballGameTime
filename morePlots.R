...
allYears.summary<-summarySE(allYears, measurevar='gameTime', groupvars=c('Year'))
...
#with 2015
ggplot(allYears.summary, aes(x=Year, y=gameTime))+
  geom_line()+
  geom_errorbar(aes(ymin=gameTime-se, ymax=gameTime+se), width=0.2)+
  geom_point(size=5)+
  theme_classic(base_size=20, base_family="Helvetica")+
...
#plot running average for each year to look for stabilization, no 2015
...
#with 2015
ggplot(allYears, aes(x=gameNumber, y=runningAvg, group=Year))+
  geom_line(aes(color=Year), size=0.75)+
  scale_color_gradient(low="red")+
  theme_classic(base_size=20, base_family="Helvetica")+
  ylab("Average Game Time (minutes)")+
...
#box and whiskers by year, by league, no 2015
ggplot(without2015, aes(x=as.factor(Year), y=gameTime))+
  geom_boxplot(aes(fill=homeLeague))+
  theme_classic(base_size=20, base_family="Helvetica")+
  ylab("Average Game Time (minutes)")+
  xlab("Year")

#with 2015
ggplot(allYears, aes(x=as.factor(Year), y=gameTime))+
  geom_boxplot(aes(fill=homeLeague))+
  theme_classic(base_size=20, base_family="Helvetica")+
  ylab("Average Game Time (minutes)")+
  xlab("Year")
...
#box and whiskers by year, by team, no 2015
...
#with 2015
ggplot(allYears, aes(x=as.factor(Year), y=gameTime, group=as.factor(Year)))+
  geom_boxplot(aes(fill=Year))+
  scale_fill_gradient(low="red")+
  theme_classic(base_size=20, base_family="Helvetica")+
  ylab("Average Game Time (minutes)")+
